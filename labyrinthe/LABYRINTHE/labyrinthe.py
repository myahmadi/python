...
pygame.init()
fenetre = pygame.display.set_mode((800, 326))
...
def testDeplacement(touche):
    ...
    if touche in ['a','z','e','q','s','d']:
        if (((touche == 'd' and d==0) or (touche == 'z' and d==1)or(touche == 'q' and d==2)or(touche == 's' and d==3)) and  tableau[y][x+1] == '1' ):
            ...
            d=(d-1)%4
        elif tableau[y][x]==tableau[13][7]:
            cle=1
        elif tableau[y][x]==tableau[7][7]:
            temp=time.clock
            cleTemp=1
        afficheMurs()

def afficheMurs():
    global temp, cle, cleTemp
    if d==0:
        afficheMurNord()
        print("Nord",end=' ')
    if d==1:
